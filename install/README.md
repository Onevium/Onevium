...
**macOS:** download [install.sh](install.sh), inspect it, then run `sh install.sh 1.2.0`. Omit the version to select the latest stable release. Apple Silicon and Intel are detected automatically. Drag the app into Applications when the DMG opens.

**Windows x64:** download [install.ps1](install.ps1), inspect it, then run `powershell -File .\install.ps1 -Version 1.2.0`. Omit `-Version` to select the latest stable release. Follow your organization's PowerShell execution policy; the helper does not bypass it.
